...
# https://github.com/slundberg/shap/blob/master/shap/plots/_benchmark.py
# add xlabel_names
xlabel_names = {
    ...
    "Sparsity (MAZ)": "Interval size", 
    "Fidelity (DA)": "% Removed features",
    "Fidelity (CD)": "% Removed features",
    "Fidelity (CR)": "% Removed features",
    "Fidelity (PCR)": "% Removed features"
}

ylabel_names = {
    # "Fidelity (DA)": "ADA", # A means 'average'
    # "Fidelity (CD)": "ACD",
    # "Fidelity (CR)": "ACR",
    "Sparsity (MAZ)": "MAZ"
} # others: default to "Model output"

def plt_benchmark(benchmark, show=True, score_name='value', **kwargs):
    ...
    if hasattr(benchmark, "__iter__"):
        ...
        if single_metric and has_curves:
            benchmark.sort(key=lambda b: -b.value_sign * b.__getattribute__(score_name))
            _scores = [b.__getattribute__(score_name) for b in benchmark]
            if kwargs.pop('scale_score', False):
                y_ini = benchmark[0].curve_y[0] 
                y_min = np.array([b.curve_y.min() for b in benchmark]).min()
                y_max = y_ini if y_min < y_ini \
                    else np.array([b.curve_y.max() for b in benchmark]).max() # initial prediction value (in case the curve is rising)
                _scores = [s / (y_max - y_min) for s in _scores]
            for i, b in enumerate(benchmark):
                plt.plot(
                    b.curve_x, b.curve_y,
                    color=method_color[b.method],
                    linewidth=2,
                    label=b.method + f" ({_scores[i]:0.3})"
                )
                ax = plt.gca()
            ax.set_xlabel(xlabel_names[metric_name], fontsize=13)
            ax.set_ylabel(ylabel_names.get(metric_name, 'Model output'), fontsize=13)
            ax.xaxis.set_ticks_position('bottom')
            ax.yaxis.set_ticks_position('left')
            ax.spines['right'].set_visible(False)
            ax.spines['top'].set_visible(False)
            plt.title(metric_name[0].upper()+metric_name[1:])
            ...
            values = np.array([b.__getattribute__(score_name) for b in benchmark])
            total_width = 0.7
            ...
            hbars = plt.barh(
                ...
                #     color=method_color[b.method],
                #     linewidth=2,
                #     label=b.method + f" ({b.__getattribute__(score_name):0.3})"
                # )
            ax = plt.gca()
        ...
        # plot a multi-metric benchmark result
            ...
            for b in benchmark:
                if max_value[b.metric] < b.value_sign * b.__getattribute__(score_name):
                    max_value[b.metric] = b.value_sign * b.__getattribute__(score_name)
                if min_value[b.metric] > b.value_sign * b.__getattribute__(score_name):
                    min_value[b.metric] = b.value_sign * b.__getattribute__(score_name)
            norm_values = {}
            for b in benchmark:
                norm_values[b.full_name] = (b.value_sign * b.__getattribute__(score_name) - min_value[b.metric]) / (max_value[b.metric] - min_value[b.metric])
            ...
            for b in benchmark:
                if b.metric == metrics[0]:
                    metric_0[b.method] = b.__getattribute__(score_name)
                elif b.metric == metrics[1]:
                    metric_1[b.method] = b.__getattribute__(score_name)
                ...
                for metric in metrics:
                    for b in benchmark:
                        if b.method == method and b.metric == metric:
                            ...
                            values.append(b.__getattribute__(score_name))
                plt.plot(
                    xs,
                    scores,
